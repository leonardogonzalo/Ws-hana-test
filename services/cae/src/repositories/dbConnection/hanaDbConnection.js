import hdbPool from "@bbel/node-hdb-pool/lib/index";
...
export default class HanaDbConnection {
    ...
    return new Promise((done, reject) => {
      ...
      if (pool) {
        ...
        pool.exec(statement, params, (error, data) => {
          console.log(
            `${name}`,
            `queryresponse ${name} ` + Math.abs(new Date().getTime() - elapsed.timer.timeStart.getTime()).toString() + " ms"
          );
          if (response === undefined) {
            ...
            if (error) {
              ...
              OPTIONS.logger.eventStop(`HANA ${name}: ${data && data.length > 0 ? data.length : 0} rows`, elapsed);
              //console.debug("DB Data Result", data);
